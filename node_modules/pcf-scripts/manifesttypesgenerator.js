...
class ManifestTypesGenerator {
    ...
    handleInternalTypes(prefix) {
        ...
        switch (prefix) {
            case 'File':
                result = 'FileProperty';
                interfaceName = 'CustomControls/Models/CustomControlExposedInterfaces';
            default:
        }
        if (result) {
    ...
    createOutputProperties() {
        const outputProperties = Object.keys(this.outputs).map(propName => {
            ...
            else if (this.outputs[propName] === 'FileProperty') {
                typeRef = ts.createTypeReferenceNode(
                /*typeName*/ 'File', 
                /*typeArguments*/ undefined);
            }
        ...
        for (const typeNS in this.internalImportsRequired) {
            if (this.internalImportsRequired.hasOwnProperty(typeNS)) {
                ...
                properties.forEach(property => {
                    importSpecifiers.push(ts.createImportSpecifier(undefined, ts.createIdentifier(property)));
                });
                importsToAdd.push(ts.createImportDeclaration(undefined, undefined, ts.createImportClause(undefined, ts.createNamedImports(importSpecifiers)), ts.createLiteral(typeNS)));
            }
        }
        return ts.createNodeArray([...importsToAdd, inputsInterface, outputsInterface], false);
    }
