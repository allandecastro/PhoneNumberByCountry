...
try {
    ...
    taskRunner.run(taskGroup_1.standardTaskGroups).then((result) => {
        ...
        const customMeasurement = {
            'Execution Time(ms)': result ? timeDiff - result.compileTime : timeDiff,
            'Compile Time(ms)': result ? result.compileTime : -1
        };
        customProperties['Success'] = !!result;
